/*
  - copy styles.css, logo.svg, browser-app.js in /navbar-app into /public 
    > DO_NOT copy index.html  
    > in /navbar-app, it just has index.html, and in public/ contains all other files


  - res.sendFile(path): when we use res.sendFile() below, it still shows errs like prev lessons (missing styles, log, js...)
    > we need to use app.use()


  - app.use():
    + use to setup middleware
...
app.use(express.static('./public')) // (2) *** setup static & middleware >> if we don't have this line, we cannot load files like css, sgv, logo...

app.get('/', (req, res) => {
  res.sendFile(path.resolve(__dirname, './navbar-app/index.html')) // (3)
})
