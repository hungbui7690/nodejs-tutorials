...
***************************
  ...
  - If you enter any command from above with a double point and the name of your scope, like the following:
  ...
  - If you don't enter a scope, by default only the migrations within the migrations directory itself will be executed. Now that we have entered the scope "test" all migrations within migrations/test will be executed.
  ...
  - <all> is a keyword in db-migrate and can thus not be used as scope name. Instead the all scope executes all scopes that exist in your migrations directory.
...
***************************
  ...
  - This config file is used to override the database or schema setting defined in database.json for a given scope. database is used for most databases, except postgres which needs the schema variable.
  - Note that database or schema must still be defined in the database.json file in order to run db-migrate up:all. The value defined in database.json will be used for any migrations in the default (top-level) scope.
