/*
  ...
  - TEST CASE
    > http://localhost:5000/?user=john >> authorize
      >> req.query = {user: 'john'}
    > http://localhost:5000/about/?user=john >> authorize
...
const authorize = require('./authorize') // (3)

app.use([logger, authorize]) // (4) multi middlewares >> logger() will run before authorize()
...
// (5) http://localhost:5000/api/items/?user=john
app.get('/api/items', (req, res) => {
  console.log(req.user) // *** we can access req.user here because this route went through authorize middleware
  res.send('/api/items')
})
