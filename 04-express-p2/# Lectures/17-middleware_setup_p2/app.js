/*
  - instead copy, we will create function logger outside of the route (this is middleware)
  ...
  - method 1: now, if we go to localhost:5000 >> though we can log, but browser will hang
    > reason: because after run function, it does not know where to go
      > we need to have next() to jump to next middleware 
      > or we to terminate by sending response to client
...
// (1) when working with middleware, we must have next() or res.send() >> otherwise, hang
